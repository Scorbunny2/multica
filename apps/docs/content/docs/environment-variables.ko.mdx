...
데몬이 에이전트 task를 시작하면 다음 변수를 런타임 프로세스에 주입합니다. 이 변수들은 실행 중인 task를 설명하며 `multica` CLI가 task 로컬 설정과 인증을 사용하도록 합니다. 값은 데몬이 관리하므로 에이전트의 사용자 지정 환경에서 같은 이름의 변수로 덮어쓸 수 없습니다.

| 변수 | task에서의 값 |
|---|---|
| `MULTICA_TOKEN` | 서버가 에이전트, task 및 워크스페이스에 바인딩한 task 범위 `mat_` API 자격 증명 |
| `MULTICA_TASK_ID` | 실행 중인 task의 ID |
| `MULTICA_TASK_CONFIG_ROOT` | 비공개 task 로컬 Multica CLI 설정 루트 |
| `MULTICA_TASK_WORKSPACES_ROOT` | 데몬이 관리하는 task 워크스페이스의 루트 |
| `MULTICA_AGENT_ID` | 할당된 에이전트의 ID |
| `MULTICA_AGENT_NAME` | 할당된 에이전트의 표시 이름 |
| `MULTICA_WORKSPACE_ID` | task가 속한 워크스페이스의 ID |
| `MULTICA_SERVER_URL` | 데몬이 선택한 Multica 서버 URL |
| `MULTICA_DAEMON_PORT` | `multica repo checkout` 같은 task 전용 명령이 사용하는 로컬 데몬 상태/API 포트 |
| `MULTICA_TASK_SLOT` | 데몬 전체 동시 실행 풀의 슬롯. 여러 task가 GPU 같은 슬롯 번호 기반 리소스를 공유할 때 사용 가능 |

`MULTICA_TOKEN`을 사용하는 요청의 작성자는 요청 헤더나 token 소유자의 `/api/me` 응답이 아니라 서버가 결정합니다. task 댓글 등의 쓰기는 할당된 에이전트와 실행 중인 task의 작업으로 표시됩니다. 사람 전용 엔드포인트는 계속 task 자격 증명을 거부합니다.

<Callout type="warning">
사용자 지정 런타임이 다른 프로세스나 중첩 에이전트 harness를 시작해도 Multica는 이 값을 두 번째 계층에 자동으로 주입하지 않습니다. 필요한 변수만 전달하세요. CLI/API 접근에는 보통 `MULTICA_TOKEN`, `MULTICA_TASK_ID`, `MULTICA_WORKSPACE_ID`, `MULTICA_SERVER_URL`이 필요합니다. token은 자식 프로세스 환경에만 두고 prompt, 로그, 저장소 파일 또는 영구 설정에 기록하지 마세요. 자식 프로세스는 부모 task의 ID와 권한을 공유하며 독립된 새 범위 ID를 받지 않습니다.
</Callout>
