...
### Variables injected into agent tasks

When the daemon starts an agent task, it injects the following variables into the runtime process. They describe the active task and let the `multica` CLI use task-local configuration and authentication. These values are owned by the daemon: entries with the same names in an agent's custom environment cannot override them.

| Variable | Value in a task |
|---|---|
| `MULTICA_TOKEN` | Task-scoped `mat_` API credential bound by the server to the agent, task, and workspace |
| `MULTICA_TASK_ID` | ID of the active task |
| `MULTICA_TASK_CONFIG_ROOT` | Private task-local Multica CLI configuration root |
| `MULTICA_TASK_WORKSPACES_ROOT` | Root of the daemon-managed task workspaces |
| `MULTICA_AGENT_ID` | ID of the assigned agent |
| `MULTICA_AGENT_NAME` | Display name of the assigned agent |
| `MULTICA_WORKSPACE_ID` | ID of the task's workspace |
| `MULTICA_SERVER_URL` | Multica server URL selected by the daemon |
| `MULTICA_DAEMON_PORT` | Local daemon health/API port used by task-only commands such as `multica repo checkout` |
| `MULTICA_TASK_SLOT` | Slot in the daemon-wide concurrency pool; useful when several tasks share slot-indexed resources such as GPUs |

The server, not request headers or the token owner's `/api/me` response, determines authorship for `MULTICA_TOKEN` requests. Writes such as issue comments are attributed to the assigned agent and active task. Human-only endpoints still reject task credentials.

<Callout type="warning">
If a custom runtime starts another process or a nested agent harness, Multica does not automatically inject these values into that second layer. Forward only the variables it needs—normally `MULTICA_TOKEN`, `MULTICA_TASK_ID`, `MULTICA_WORKSPACE_ID`, and `MULTICA_SERVER_URL` for CLI/API access—and keep the token in the child process environment. Do not write it to a prompt, log, repository file, or persistent configuration. The child shares the parent task's identity and permissions; it does not receive a new independently scoped identity.
</Callout>
